...
//			final int maxFilesPerFolder=10;
	...
	public LanguageJsonWriter(String outputFolder) {
		super();
		this.outputFolder = outputFolder;
	}

	public void write(LanguageInRecord res) throws IOException {
		if (writer == null) {
			fileCounter++;
			if (fileCounter == 1) {
				folderCounter++;
			}
			File folder = new File(outputFolder, "language_export_" + String.format("%03d", folderCounter));
			if (!folder.exists())
				folder.mkdirs();
			writer = new FileWriterWithEncoding(new File(folder, "language_export_" + String.format("%03d", folderCounter)
