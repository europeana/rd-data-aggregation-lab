...
public class PatternDateExtractorYyyyMmDd implements DateExtractor {
...
//  )"	
	...
	public Match extract(String inputValue) {
		...
		if(m.matches())
			return new Match(MatchId.YYYY_MM_DD, inputValue, m.group(1)+"-"+m.group(2)+"-"+m.group(3));
		m=patYyyyMmDdX.matcher(inputValue); 
		if(m.matches()) 
			return new Match(MatchId.xYYYY_MM_DDx, inputValue, m.group(2)+"-"+m.group(3)+"-"+m.group(4));
		return null;
	}
