...
public class MapOfMapsOfInts<K1, K2> implements Serializable{
		...
		public List<Entry<K1, List<Entry<K2,Integer>>>> getAllEntries(){
			ArrayList<Entry<K1, List<Entry<K2,Integer>>>>ret=new ArrayList<Map.Entry<K1,List<Entry<K2,Integer>>>>(map.size()) ;
			ArrayList<Entry<K1, MapOfInts<K2>>> allEntries=new ArrayList<>(map.entrySet());
			for(Entry<K1, MapOfInts<K2>> e1: allEntries) {
				ret.add(new AbstractMap.SimpleEntry<K1, List<Entry<K2,Integer>>>(e1.getKey(), e1.getValue().getSortedEntries()));
			}
			return ret;
		}
		...
		public static void writeCsv(MapOfMapsOfInts<?, ?> map, Appendable csvWrite) throws IOException {
			CSVPrinter printer=new CSVPrinter(csvWrite, CSVFormat.DEFAULT);
			@SuppressWarnings("rawtypes")
			ArrayList<Entry<?, MapOfInts<?>>> allEntries=new ArrayList(map.map.entrySet());
			for(Entry<?, MapOfInts<?>> e1: allEntries) {
				boolean first=true;
				for(Object k2 :  e1.getValue().keySet()) {
					if(first) {
						first=false;
						printer.printRecord(e1.getKey().toString(), k2.toString(), e1.getValue().get(k2));
					} else
						printer.printRecord("", k2.toString(), e1.getValue().get(k2));
				}
			}
			printer.close();
		}
